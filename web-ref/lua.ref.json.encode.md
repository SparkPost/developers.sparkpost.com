...
`json.encode` converts the `array` expression so that it can be assigned to a JSON object field. You can pass a variable or a literal to this function. The output of the print statement in [Example 15.38, “json.encode example”](lua.ref.json.encode#lua.ref.json.encode.example "Example 15.38. json.encode example") is as follows:
