...
function serializeHit(hit) {
  if (hit.actionName) {
    return { href: hit.objectID, title: hit.actionName, section: hit.actionName === hit.resGroupName ? hit.resName : hit.resGroupName }
  }
  else if (hit.resName) {
    return { href: hit.objectID, title: hit.resName, section: hit.resGroupName }
  }
  else if (hit.resGroupName) {
    return { href: hit.objectID, title: hit.resGroupName, section: 'SparkPost API' }
  }
  else {
    return { href: hit.objectID, title: hit.sectionName, section: 'SparkPost API' }
  }
}
...
const Hits = connectStateResults(
  ({ searchState, searchResults, hitComponent: Hit, active, activeIndex, setActiveIndex }) => (
    active && searchResults && searchState.query && searchResults.nbHits !== 0 ?
      <SearchResults>{searchResults.hits.map((hit, i) => (
        <Hit key={i} hit={hit} active={i === activeIndex} setActive={() => {
          setActiveIndex(i)
        }} />
  ...
  render() {
    return (
      <InstantSearch
        ...
        }} onFocus={this.toggleActive} onBlur={this.toggleActive} />
        <Hits
